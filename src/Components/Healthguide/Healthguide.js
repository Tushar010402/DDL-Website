...
const Healthguide = () => {
    ...
    const fetchBlogs = async () => {
        try {
            const response = await axios.get('https://api.dangsccg.co.in/api/get_blogs/');
    ...
    const fetchArticles = async () => {
        try {
            const response = await axios.get('https://api.dangsccg.co.in/api/get_articles/');
            if (response.status === 200) {
                    ...
                    <SwiperSlide key={blog.id}>
                        <div className={styles['hg-card']}>
                            <div className={styles['hg-image-wrapper']}>
                                <img
                                    src={`https://api.dangsccg.co.in${blog.blog_thumbnail_image}`}
                ...
                {articles.map((article) => (
                    <SwiperSlide key={article.id}>
                        <div className={styles['hg-card']}>
                            <div className={styles['hg-image-wrapper']}>
                                <img
                                    src={`https://api.dangsccg.co.in${article.article_thumbnail_image}`}
